# 文档地图

本目录按使用场景分类，避免所有文档平铺在一层。新增文档时优先放入最贴近用途的分类，并在本页补入口。

## Planning

面向目标、路线和约束。

- [任务说明](planning/mission_brief.md)
- [研发路线](planning/roadmap.md)
- [车机 ROS2 迁移技术路线](planning/vehicle_ros2_migration_route.md)
- [重量预算](planning/weight_budget.md)

## Design

面向系统结构和技术基线。

- [总体架构](design/architecture.md)
- [系统基线](design/system_baseline.md)

## Engineering

面向日常协作、命令和工程流程。

- [团队协作](engineering/team_workflow.md)
- [终端命令](engineering/terminal_commands.md)

## Validation

面向构建、mock/sim、车机、硬件和端到端流程的验证记录。

- [Mock/sim 验证记录](validation/mock_bringup_validation.md)
- [车机 Noetic 基线验证记录](validation/vehicle_noetic_baseline_validation.md)

## References

面向第三方依赖、厂商资料和来源归档。

- [第三方依赖](references/third_party_dependencies.md)
- [AgileX 平台参数归档](references/vendor_agilex_platform_parameters.md)
- [tzb2026 厂商基础代码扫描分析报告](references/vendor_tzb2026_code_audit.md)
- [scout_ros2 与 tzb2026 Scout 基础代码对比](references/scout_ros2_vs_tzb2026_scout_ros.md)
