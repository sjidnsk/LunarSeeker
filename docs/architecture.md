...
- 移动自主组维护 `/map`、`/tf`、`/odom` 或 `/odometry/filtered`、导航目标状态和 `/cmd_vel` 控制输出；上层任务逻辑不直接读取底层 CAN 或传感器私有接口。
- 视觉感知组对移动自主组和机械臂组交付 `/target_detections`，输出目标类别、置信度、3D 位姿、frame、状态和可采样标记；下游模块不直接依赖检测模型内部输出。
- 机械臂组对任务状态机交付机械臂动作成功 / 失败状态，任务状态机不直接管理单关节控制细节。
- 系统集成与接口协调牵头人维护 `/mission/state` 和 `/execute_mission`；牵头人未指定前，由每周接口评审共同确认，各模块通过任务状态而不是临时脚本串联。
