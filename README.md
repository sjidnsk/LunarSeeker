...
- 第三方依赖: [docs/third_party_dependencies.md](docs/third_party_dependencies.md)
...
第三方驱动源码不直接提交到本仓库，后续在 ROS2 环境中通过 `vcs import . < dependencies.repos` 拉取 `scout_ros2`、`ugv_sdk`、`piper_ros` humble 和 `piper_sdk`。当前导入版本和验证状态见 [docs/third_party_dependencies.md](docs/third_party_dependencies.md)。
...
- `scout_ros2`、`ugv_sdk`、`piper_ros`、`piper_sdk`: 已写入 [dependencies.repos](dependencies.repos)，当前 commit 记录见 [docs/third_party_dependencies.md](docs/third_party_dependencies.md)，仍需在硬件 bringup 阶段验证后锁定。
